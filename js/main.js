...
function getAllPossibleCoordsRook(pieceCoord) {
  ...
  for (var i = pieceCoord.i + 1; i < gBoard.length; i++) {
    if (!isEmptyCell({i: i, j: pieceCoord.j})) break;
    res.push({i: i, j: pieceCoord.j});
  }

  for (var i = pieceCoord.i - 1; i >= 0; i--) {
    console.log({i: i, j: pieceCoord.j});
    if (!isEmptyCell({i: i, j: pieceCoord.j})) break;
    res.push({i: i, j: pieceCoord.j});
  }

  for (var j = pieceCoord.j + 1; i < gBoard[0].length; j++) {
    if (!isEmptyCell({i: pieceCoord.i, j: j})) break;
...
function getAllPossibleCoordsQueen(pieceCoord) {
  ...
  var horizontal = getAllPossibleCoordsRook(pieceCoord);
  var diagonal = getAllPossibleCoordsBishop(pieceCoord);

  res = [...horizontal, ...diagonal];
  console.log(res);
  return res;
}
...
function getAllPossibleCoordsKnight(pieceCoord) {
  ...
  for (var i = pieceCoord.i - 2; i < pieceCoord.i; i++) {
    if (i < 0 || i > gBoard.length - 1) continue;
    if (isEmptyCell({i, j: pieceCoord.j - add}))
      res.push({i, j: pieceCoord.j - add});
    if (isEmptyCell({i, j: pieceCoord.j + add}))
      res.push({i, j: pieceCoord.j + add});
    add++;
  }
  ...
  for (var i = pieceCoord.i + 2; i > pieceCoord.i; i--) {
    if (i < 0 || i > gBoard.length - 1) continue;
    if (pieceCoord.j - add < 0 || pieceCoord.j - add > gBoard.length - 1)
      continue;
    if (isEmptyCell({i, j: pieceCoord.j - add}))
      res.push({i, j: pieceCoord.j - add});
    if (isEmptyCell({i, j: pieceCoord.j + add}))
